...
class CFN(SingleStateRecurrentLayerBase):
    ...
    Outputs: output, h_n
        - **output**: tensor of shape
          ...
          features `(h_t)` from the last layer of the CFN, for each `t`.
...
class CFNCell(SingleStateCellBase):
    ...
    def forward(self, inp: Tensor, state: Optional[Tensor] = None) -> Tensor:
        ...
        rec_exp = b_state @ self.weight_hh.t() + self.bias_hh
        ...
        h_new = horizontal_gate * torch.tanh(b_state) + vertical_gate * torch.tanh(
            input_exp_3
        )
