...
class MiRU1(BaseSingleRecurrentLayer):
    ...
    where :math:`\lambda` is a learned parameter, :math:`\sigma` is the
...
class MiRU1Cell(BaseSingleRecurrentCell):
    ...
    where :math:`\lambda` is a learnable parameter vector of shape
    ``(hidden_size,)``, :math:`\circ` is element-wise product, and
    ...
    Variables:
        ...
        lambda_: Learnable mixing parameter of shape ``(hidden_size,)``,
            initialised to 0.5.
    ...
    def __init__(
        ...
        # Learnable lambda parameter, initialised to 0.5
    ...
    def forward(
        ...
        new_state = lam * state + (1.0 - lam) * candidate
...
class MiRU2(BaseSingleRecurrentLayer):
    ...
    where :math:`\lambda` and :math:`\theta` are learned parameters, and
...
class MiRU2Cell(BaseSingleRecurrentCell):
    ...
    where :math:`\lambda` and :math:`\theta` are learnable parameter vectors
    of shape ``(hidden_size,)``, both initialised to 0.5, and :math:`\circ`
    ...
    Variables:
        ...
        lambda_: Learnable mixing parameter of shape ``(hidden_size,)``,
            initialised to 0.5.
        theta: Learnable hidden-state scaling parameter of shape
            ``(hidden_size,)``, initialised to 0.5.
    ...
    def __init__(
        ...
        # Learnable lambda and theta, both initialised to 0.5
        self.lambda_ = nn.Parameter(
            torch.full((hidden_size,), 0.5, device=device, dtype=dtype)
        )
