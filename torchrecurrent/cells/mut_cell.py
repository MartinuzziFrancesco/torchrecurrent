...
class MUT1(SingleStateRecurrentLayerBase):
    ...
    Outputs: output, h_n
        - **output**: tensor of shape
          ...
          features `(h_t)` from the last layer of the MUT1, for each `t`.
...
class MUT2(SingleStateRecurrentLayerBase):
    ...
    Outputs: output, h_n
        - **output**: tensor of shape
          :math:`(L, N, H_{out})` when ``batch_first=False`` or
          :math:`(N, L, H_{out})` when ``batch_first=True`` containing the output
          features `(h_t)` from the last layer of the MUT2, for each `t`.
        - **h_n**: tensor of shape :math:`(\text{num_layers}, N, H_{out})`
          containing the final
          hidden state for each element in the sequence.
...
class MUT3(SingleStateRecurrentLayerBase):
    ...
    Outputs: output, h_n
        - **output**: tensor of shape
          :math:`(L, N, H_{out})` when ``batch_first=False`` or
          :math:`(N, L, H_{out})` when ``batch_first=True`` containing the output
          features `(h_t)` from the last layer of the MUT3, for each `t`.
        - **h_n**: tensor of shape :math:`(\text{num_layers}, N, H_{out})`
          containing the final
          hidden state for each element in the sequence.
