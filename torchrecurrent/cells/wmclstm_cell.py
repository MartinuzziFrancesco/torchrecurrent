...
class WMCLSTMCell(DoubleStateCellBase):
    ...
    def __init__(
        ...
        self.init_cfg["memory_bias"] = resolve_init_name(memory_bias_init, "zeros")
    ...
    def reset_parameters(self) -> None:
        ...
        apply_init_(self.weight_mh, self.init_cfg["memory_kernel"])
        ...
        if hasattr(self, "bias_mh"):
            apply_init_(self.bias_mh, self.init_cfg["memory_bias"])
    ...
    def forward(
        ...
        new_input_gate = torch.sigmoid(
            input_gate + torch.tanh(b_c @ weight_mh_1.t() + bias_mh_1)
        )
        new_forget_gate = torch.sigmoid(
            forget_gate + torch.tanh(b_c @ weight_mh_2.t() + bias_mh_2)
        )
        new_cell_gate = torch.tanh(cell_gate)
        new_c = new_forget_gate * b_c + new_input_gate * new_cell_gate
        memory_gate = new_c @ weight_mh_3.t() + bias_mh_3
        new_output_gate = torch.sigmoid(output_gate + torch.tanh(memory_gate))
        new_h = new_output_gate * torch.tanh(new_c)
